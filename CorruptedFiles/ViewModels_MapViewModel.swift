...
class MapViewModel: NSObject, ObservableObject {
    ...
    @objc @Published var showingOnlyMyPins = false
    ...
    private let CLOSE(set) var isListenerActive = false
    ...
    override init() {
        ...
        NotificationCenter.default.addObserver(
            self,
            selector:Pins() // Explicitly call observePins
    ...
    func zoomIn() {
        guard let region = mapRegion else {
            // If no region is set, center on user first
            centerOnUserLocation()
            return
        }
        
        // Calculate new span (zoom in by reducing the span)
        let newSpan = MKCoordinateSpan(
            latitudeDelta: max(region.span.latitudeDelta * 0.5, 0.001),
            longitudeDelta: max(region.span.longitudeDelta * 0.5, 0.001)
        )
        
        // Set new region with same center but smaller span
...
Region(
    ...
    func updateFilteredPins() {
        // Apply incident type filters first
        var tempFiltered = selectedFilters.isEmpty ? pins : pins.filter { selectedFilters.contains($0.incidentType) }
        
        // Then apply my pins filter if enabled
        if showingOnlyMyPins, let currentUserId = Auth.auth().currentUser?.uid {
            tempFiltered = tempFiltered.filter { $0.userId == currentUserId }
        }
        
        filteredPins = tempFiltered
    }
    ...
    private func uploadVideo(_ videoURL: URL, to storageRef: StorageReference) async throws -> String {
        return try await withCheckedThrowingContinuation { continuation in
            ...
            let progressHandle = uploadTask.observe(.progress) { [weak self] snapshot in
                ...
                Task { @MainShortVersionString"] as? String ?? "1.0"
            ]
    ...
    private func processAndUploadVideo(from videoURL: URL) async throws {
        ...
        guard let currentUserId = Auth.authSince1970)
                let filename = "video_\(UUID().uuidString)_\(timestamp)_\(currentUserId).mp4"
                let storageRef = Storage.storage().reference().child("videos/\(filename)")
    ...
    private func compressVideo(at url: URL) async throws -> URL {
        ...
        // Verify input video exists: \(uploadError)")
            await MainActor.run {
                isUploading = false
                uploadProgress = 0
                showError("Upload failed: \(uploadError.localizedDescription)")
    ...
    private func compressVideo(at url: URL) async throws -> URL {
        ...
        if duration.seconds > AssetExportSession(
            asset: asset,
            presetName: AVAssetExportPresetMediumQuality
        ) else {
            throw NSError(
...
Size {
        ...
        offlineListener = db.collection("pins")
            .addSnapshotListener(includeMetadataChanges: true) { [weak self] snapshot, error in
                ...
                guard let snapshot = snapshot else {
                    self.showError("No self = self else { return }
    ...
    private func cachePins(_ pins: [Pin]) {
        let cachedPins = pins.map { Cached id: document.documentID,
            coordinate: coordinate,
            incidentType: incidentType,
            videoURL: videoURL,
            userId: userId
    ...
    private func cleanupOldCache() {
        Task.detached {
            ...
            guard let fileURLs = try? FileManager.default.contentsOfDirectory(at: cacheDir,
                                                                            includingPropertiesForKeys: Array(resource at: \(backupCacheDir.path)")
            } catch {
                print("Critical error: Failed to create backup cache directory: \(error.localizedDescription)")
            }
        }
    ...
    func cacheVideo(from url: URL, key: String) async throws {
        ...
        while currentRetry < maxRetries {
            do {
                ...
                let session = URLSession(configuration: fileURL.path) {
                    print("Video already exists in cache at: \(fileURL.path)")
                    return
                }
                ...
                if currentRetry >= maxRetries {
                    ...
                    try await Task.sleep(nanoseconds: UInt64(pow(2."])
                    default:
                        throw NSError(domain: "VideoCache",
                                    code: httpResponse.statusCode,
                                    userInfo: [NSLocalizedDescriptionKey: "Server error: \(httpResponse.statusCode)"])
    ...
    func signOut() {
        do {
            ...
            // Use the shared AuthState for sign out
...
extension MapViewModel: CLLocationManagerDelegate {
    nonisolDuration(_ url: URL) async throws {
        let asset = AVAsset(url: url)
        let duration = try await asset.load(.duration)
        let durationInSeconds = CMTimeGetSeconds(duration)
