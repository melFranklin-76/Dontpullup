...
struct MapView: UIViewRepresentable {
    ...
    func makeUIView(context: Context) -> MKMapView {
        ...
        // We'll request location permission in updateUIView instead to avoid main thread blocking
    ...
    func updateUIView(_ mapView: MKMapView, context: Context) {
        ...
        if !context.coordinator.hasLoggedMapStatus {
            print("DEBUG: Map view updating - Current mapType: \(mapView.mapType.rawValue)")
            print("DEBUG: User location visible: \(mapView.showsUserLocation)")
            
            // Location permission flow will be handled elsewhere (placeholder removed)
    ...
    class Coordinator: NSObject, MKMapViewDelegate {
        ...
        @objc func handleLongPress(_ gesture: UILongPressGestureRecognizer) {
            // Only handle the beginning of the gesture and if edit mode is active
            guard gesture.state == .began, parent.viewModel.isEditMode else { return }
            ...
            // Call the ViewModel to handle verification and potential presentation
            parent.viewModel.initiatePinDropVerification(at: validCoordinate)
            
            // Remove the direct setting of state here - ViewModel handles it now
